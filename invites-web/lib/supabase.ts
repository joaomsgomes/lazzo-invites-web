...
/**
 * If avatar is a relative storage path, returns a short-lived signed URL (service role).
 * Absolute URLs are returned unchanged. Returns null if signing is not possible.
 */
export async function signAvatarUrlIfRelative(avatarUrl: string | null): Promise<string | null> {
  if (!avatarUrl) return null;
  if (isAbsoluteUrl(avatarUrl)) return avatarUrl;
  const serviceClient = createServiceSupabase();
  if (!serviceClient) return null;
  const map = await buildSignedAvatarMap(serviceClient, [avatarUrl]);
  return map.get(avatarUrl) ?? null;
}
